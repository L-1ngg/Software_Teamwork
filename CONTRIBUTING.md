...
- 主仓库日常集成分支是 `develop`。
- 禁止向 `main` 发起日常开发 PR。
- 禁止直接在主仓库分支上提交功能、修复或文档修改。
- 每次修改必须从主仓库最新 `develop` 创建个人 fork 中的独立分支。
- PR 必须指向主仓库 `develop`，并添加对应小组 label。
- `main` 只用于发布，由维护者从 `develop` 做发布合并。

## 小组 Label

PR 必须至少包含一个小组 label。当前仓库使用以下小组 label：
...
`Frontend` 是领域 label，可作为补充标签使用，但不能替代小组 label。

维护者新增小组时，需要同时更新：

- GitHub 仓库 label
- `.github/workflows/pr-guard.yml` 中的 `REQUIRED_GROUP_LABELS`
- 本文档的小组 label 表
