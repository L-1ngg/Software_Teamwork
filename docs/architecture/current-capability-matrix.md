...
| Gateway active route proxy 和 session cache | 部分实现 | `services/gateway/` active proxy matrix、Redis session cache、Auth routes；Knowledge document lifecycle、chunks/content 和 `knowledge-queries` active routes 已转为 owner proxy。 | Gateway OpenAPI 是前端权威契约。 | 真实 Redis/downstream 跨服务 smoke 未自动化。 | #153、#199、#125 |
...
| Knowledge 知识库 CRUD、上传和 ingestion 写入链路 | 部分实现 | Knowledge PostgreSQL repository、知识库 CRUD、文档上传、File handoff、asynq ingestion 入队、Parser Service client、chunker、embedding、chunk 持久化、Qdrant/in-memory vector index 写入、document lifecycle、chunks/content 和 `knowledge-queries` 检索。 | Gateway Knowledge active paths；Knowledge service OpenAPI。 | 真实 File/Parser/Redis/Qdrant/AI Gateway 端到端 smoke、真实 AI Gateway embedding/rerank provider 接入和并发/外部副作用一致性仍需补齐。 | #152、#200、#226 |
| Parser Runtime | 部分实现 | `services/parser/` Python/FastAPI runtime、`docs/services/parser/api/internal.openapi.yaml`、`services/parser/api/openapi.yaml` 实现本地副本、service-token auth、PaddleOCR optional extra、Dockerfile 和 Knowledge Parser service client。 | Parser 内部 `/internal/v1/parsed-documents` API；不通过 gateway 暴露。 | 真实 PaddleOCR 模型 smoke、部署资源配置和 Knowledge/File/Redis 全链路 smoke 仍需补齐。 | #83、#226 |
...
| 本地联调环境 | 部分实现 | 根 `deploy/docker-compose.yml` 已提供本地/演示基线，包含 PostgreSQL、Redis、Parser、服务串联和 `seed-local` / `seed-local-ai` 基础 seed；Compose 会启动 Qdrant/MinIO 容器，但默认 Knowledge 走 in-memory vector index、File 走 local storage；QA 和 Document 也有服务级 Compose。 | 本地运行手册见 `deploy/README.md` 和 `docs/runbooks/local-integration.md`。 | 现有 seed data 覆盖本地登录、基础报告类型、示例知识库和 AI profile placeholder；Qdrant/MinIO 业务接入、跨服务验收数据覆盖、生产部署基线、统一跨服务 smoke 和一键 E2E 验收脚本仍缺。 | #125、#150 |
...
5. 测试策略要继续区分当前 CI、PR 前建议和缺口；路径过滤 CI 已覆盖前端、服务级 Go、Docker/Compose config，但后端跨服务 E2E smoke 仍未落地。
