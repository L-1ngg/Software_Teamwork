...
本文档定义 `qa` 服务的前端接口草案和 Agent Host 设计目标，用于把智能问答接口适配到当前 gateway 契约体系。本文档基于以下来源整理：
...
> 当前状态：本文档是 `qa` 服务接口草案和设计目标。`docs/api/gateway.openapi.yaml` 仍将 QA 相关接口登记在 `x-missing-contracts` 中；前端和后端在把这些接口升级为稳定契约前，需要同步更新 OpenAPI。
...
- 从 `x-missing-contracts` 中移除或收窄已稳定的 QA placeholder，保留未稳定的 Agent Run、MCP tool-call 和配置缺口。
